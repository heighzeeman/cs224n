...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    M = softmax(np.matmul(outsideVectors, centerWordVec))
    gradCenterVec = -outsideVectors[outsideWordIdx]
    loss = -np.log(M[outsideWordIdx])
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in range(outsideVectors.shape[0]):
        gradCenterVec += outsideVectors[i] * M[i]
        gradOutsideVecs[i] = M[i] * centerWordVec
        if i == outsideWordIdx:
            gradOutsideVecs[i] -= centerWordVec
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    #print("INDICES SAMPLED =", indices)

    ### YOUR CODE HERE (~10 Lines)
    ### Please use your implementation of sigmoid in here.
    U = -outsideVectors[indices]
    #print("U SHAPE =", U.shape, "OV SHAPE:", outsideVectors.shape)
    #print("U B4 =", U)
    U[0] = -U[0]
    #print("U AFT =", U)
    sigs = sigmoid(np.matmul(U, centerWordVec))
    C = 1.0 - sigs
    #print("C =", C)
    loss = -np.sum(np.log(sigs))
    gradCenterVec = -U[0] * C[0]
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in range(1, K + 1):
        gradCenterVec -= U[i] * C[i]
        #counts = np.count_nonzero(np.array(indices) == indices[i])
        #print("COUNTS: Index =", i, "translated index =", indices[i], "occurrences =", counts)
        gradOutsideVecs[indices[i]] += C[i] * centerWordVec
    gradOutsideVecs[indices[0]] = -C[0] * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    curr_word_idx = word2Ind[currentCenterWord]
    for i in range(len(outsideWords)):
        a, b, c = word2vecLossAndGradient(centerWordVectors[curr_word_idx], word2Ind[outsideWords[i]], outsideVectors, dataset)
        loss += a
        gradCenterVecs[curr_word_idx] += b
